...
SDL_Surface* rotateSurface90Degrees(SDL_Surface* src, int numClockwiseTurns) 
{
    ...
    dst = SDL_CreateSurface(newWidth, newHeight, src->format->BitsPerPixel);
    if(!dst) {
...
SDL_Surface *rotozoomSurfaceXY(SDL_Surface * src, double angle, double zoomx, double zoomy, int smooth)
{
	...
	*/
	...
	if ((is32bit) || (src->format->BitsPerPixel == 8)) {
		...
		*/
		rz_src =
			SDL_CreateSurface(src->w, src->h, 32);
	...
	if (fabs(angle) > VALUE_LIMIT) {
		...
		*/
		...
		if (is32bit) {
			...
			*/
			rz_dst =
				SDL_CreateSurface(dstwidth, dstheight + GUARD_ROWS, 32);
		} else {
			/*
			* Target surface is 8bit 
			*/
			rz_dst = SDL_CreateSurface(dstwidth, dstheight + GUARD_ROWS, 8);
		}
...
SDL_Surface *shrinkSurface(SDL_Surface *src, int factorx, int factory)
{
	...
	*/
	...
	if ((is32bit) || (src->format->BitsPerPixel == 8)) {
		...
		* New source surface is 32bit with a defined RGBA ordering 
		*/
		rz_src = SDL_CreateSurface(src->w, src->h, 32);
		if (rz_src==NULL) {
			haveError = 1;
