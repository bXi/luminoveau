...
void SpawnRocks(int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (std::abs(p.x - width * 0.5f) < 120 && std::abs(p.y - height * 0.5f) < 120)
            p.x += 240;
...
Lumi::Result AppIterate(void *appstate) {
    ...
    // ── Ship control ───────────────────────────────────────────────────────────
    if (Btn(Buttons::LEFT, Action::HELD))
        shipAngle -= turnSpeed * dt;
    if (Btn(Buttons::RIGHT, Action::HELD))
        shipAngle += turnSpeed * dt;

    vf2d heading = { std::cos(shipAngle), std::sin(shipAngle) };
    if (Btn(Buttons::UP, Action::HELD))
        ship.vel += heading * (thrust * dt);
    ...
    for (Body &r : rocks)
        Draw::Circle(r.pos, r.radius, { 170, 170, 190, 255 });
    for (Body &b : bullets)
        Draw::CircleFilled(b.pos, 3.0f, { 255, 240, 160, 255 });
