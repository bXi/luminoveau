...
Lumi::Result AppIterate(void *appstate) {
    ...
    for (InputDevice *dev : Input::GetAllInputs()) {
        ...
        if (dev->Is(Buttons::SHOOT, Action::HELD))
            firing = true;
    }
    ...
    if (vc.GetRightJoystickMagnitude() > 0.0f) {
        aimDir = vc.GetRightJoystickDirection();
        firing = true;
    }
    ...
    bullets.erase(std::remove_if(bullets.begin(), bullets.end(),
                      [](Ent &b) { return Off(b.pos); }),
        bullets.end());

    // Enemies steer toward the player.
    for (Ent &e : enemies) {
        e.vel = (player - e.pos).norm() * enemySpeed;
        e.pos += e.vel * dt;
    }
    ...
    for (Ent &e : enemies)
        Draw::CircleFilled(e.pos, e.radius, { 220, 90, 90, 255 });
    for (Ent &b : bullets)
        Draw::CircleFilled(b.pos, b.radius, { 255, 240, 150, 255 });
