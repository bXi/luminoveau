...
int ComputeReference(DD cx, DD cy) {
    ...
    for (n = 0; n < MAXREF; n++) {
        ...
        zx = nx; zy = ny;
        if (zx.hi * zx.hi + zy.hi * zy.hi > 1e12) { n++; break; }   // reference escaped
    }
    return n;
}
...
Lumi::Result AppIterate(void* appstate) {
    ...
    if (Input::MouseButtonPressed(SDL_BUTTON_LEFT))  { panning = true;  lastMtx = mtx; lastMty = mty; }
    if (Input::MouseButtonReleased(SDL_BUTTON_LEFT))   panning = false;
    if (panning && mbScale > 0.0) {
        double dx = (mtx - lastMtx) / mbScale, dy = (mty - lastMty) / mbScale;
        mbCenterX = ddSub(mbCenterX, ddSet(dx)); mbCenterY = ddSub(mbCenterY, ddSet(dy));
        mbTargetCenterX = ddSub(mbTargetCenterX, ddSet(dx));
        mbTargetCenterY = ddSub(mbTargetCenterY, ddSet(dy));
        lastMtx = mtx; lastMty = mty;
    }

    float factor = 0.0f;
    if (Input::MouseScrolledUp())   factor = 1.15f;
    if (Input::MouseScrolledDown()) factor = 1.0f / 1.15f;
