...
EffectAsset MakeEffect(const char *frag) {
    ...
    return EffectHandler::Create(v, f);
}
...
Lumi::Result AppIterate(void *appstate) {
    ...
    if (grayscaleOn)
        stack.push_back(&grayscale);
    if (waveOn)
        stack.push_back(&wave);
    if (vignetteOn)
        stack.push_back(&vignette);
    ...
    for (int i = 0; i < (int)stack.size(); i++) {
        if (i == 0)
            Draw::SetEffect(*stack[i]);
        else
            Draw::AddEffect(*stack[i]);
    }

    for (Blob &b : blobs)
        Draw::CircleFilled(b.pos, b.r, b.col);
