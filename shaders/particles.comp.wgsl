// Particle compute shader (WGSL) — port of particles.comp.hlsl
// Binding layout:
//   group(0) binding(0) : uniforms (ComputeUniforms)
//   group(1) binding(0) : systems  (read-only GPUParticleSystem[])
//   group(1) binding(1) : colliders(read-only GPUCollider[])
//   group(2) binding(0) : particles(read-write GPUParticle[])
...
fn main(@builtin(global_invocation_id) dispatchID: vec3<u32>) {
    ...
    // bit 1 = custom compute owns this system — skip built-in logic
    ...
    if (posAndLife.w > 0.0) {
        ...
        if (respawnTimer <= 0.0) {
            ...
            var spawnPos   = sys.spawnPos.xyz + vec3<f32>(cos(spawnAngle) * spawnR,
            ...
            let maxLife = mix(sys.lifetimeMin, sys.lifetimeMax,
                              biasedSample(seed + 4u, sys.lifetimeBias));

            let startSize = mix(sys.sizeStartMin, sys.sizeStartMax,
                                biasedSample(seed + 5u, sys.sizeStartBias));

            let endSize = mix(sys.sizeEndMin, sys.sizeEndMax,
                              biasedSample(seed + 6u, sys.sizeEndBias));

            let startAngle = hash(seed + 7u) * 6.2831853;
            let angVel = mix(sys.angVelMin, sys.angVelMax,
