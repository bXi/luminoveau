...
void ArrayRenderPass::render(
    ...
    assert(render_pass);
    {
        ...
        for (const auto &renderable: renderQueue) {
            ...
            float     angle    = fmod((float)Window::GetRunTime() * 10.f, 360.f);  // Rotation angle in degrees
            glm::vec2 pivot    = { 0.5f, 0.5f };                    // Normalized pivot (center)
            float     z_index  = static_cast<float>(Renderer::GetZIndex()) / static_cast<float>(INT32_MAX);

            // IMPORTANT: Compute pivot_offset in the *scaled* space!
            glm::vec2 pivot_offset = (size * scale) * pivot;  // element-wise multiplication
            ...
            // Combine them.
            // Since the right-most matrix is applied first, the order is:
            // 1. Scale, 2. Translate by -pivot, 3. Rotate, 4. Translate by +pivot, 5. Translate to final position.
...
void ArrayRenderPass::createShaders() {
