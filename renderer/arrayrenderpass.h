...
class ArrayRenderPass : public RenderPass {
    struct Uniforms {
        glm::mat4 camera;
        glm::mat4 model;
        glm::vec2 flipped;
...
public:
    ArrayRenderPass(const ArrayRenderPass &) = delete;

    ArrayRenderPass &operator=(const ArrayRenderPass &) = delete;

    ArrayRenderPass(ArrayRenderPass &&) = delete;

    ArrayRenderPass &operator=(ArrayRenderPass &&) = delete;

    explicit ArrayRenderPass(SDL_GPUDevice *m_gpu_device) : RenderPass(m_gpu_device) {
    }
