 ...
 *
 * Edited by 2024 Dennis Meinen for SDL3 compatibility.
 *
 ...
 *   CRT on windows and with SDL you're better off using SDL_IOStream, incl. SDL_RWFromFile()
...
#include <SDL3/SDL.h>
  ...
  #define STBI_NO_STDIO // don't need STDIO, will use SDL_IOStream to open files
#endif
#include "stb_image.h"
...
// loads an image file into a RGB(A) SDL_Surface from a seekable SDL_IOStream (src)
// if you set freesrc to non-zero, SDL_RWclose(src) will be executed after reading.
// Returns NULL on error, use SDL_GetError() to get more information.
SDL_STBIMG_DEF SDL_Surface* STBIMG_Load_RW(SDL_IOStream* src, int freesrc);
...
// loads an image file into a RGB(A) SDL_Texture from a seekable SDL_IOStream (src)
// if you set freesrc to non-zero, SDL_RWclose(src) will be executed after reading.
// Returns NULL on error, use SDL_GetError() to get more information.
SDL_STBIMG_DEF SDL_Texture*
STBIMG_LoadTexture_RW(SDL_Renderer* renderer, SDL_IOStream* src, int freesrc);
...
#if SDL_MAJOR_VERSION > 1
// loads an image file into a RGB(A) SDL_Surface from a SDL_IOStream (src)
...
SDL_STBIMG_DEF SDL_Texture* STBIMG_LoadTexture_RW_noSeek(SDL_Renderer* renderer, SDL_IOStream* src, int freesrc);
#endif // SDL_MAJOR_VERSION > 1
...
static SDL_Surface* STBIMG__CreateSurfaceImpl(STBIMG__image img, int freeWithSurface)
{
	...
	auto format = (img.format == STBI_rgb) ? SDL_PIXELFORMAT_RGB24 : SDL_PIXELFORMAT_ABGR8888;
...
static int STBIMG__io_read(void* user, char* data, int size)
{
	...
	int ret = SDL_ReadIO(io->src, data, size);
	if(ret == 0)
	{
...
SDL_STBIMG_DEF SDL_Surface* STBIMG_Load_RW(SDL_IOStream* src, int freesrc)
{
	...
	if(SDL_SeekIO(src, srcOffset, SDL_IO_SEEK_SET) < 0)
	{
	...
	else if(img.data == NULL)
	{
		...
		SDL_SeekIO(src, srcOffset, SDL_IO_SEEK_SET);
	}
...
SDL_STBIMG_DEF SDL_Surface* STBIMG_Load_RW_noSeek(SDL_IOStream* src, int freesrc)
{
	...
	if(SDL_ReadIO(src, buf, fileSize) > 0)
	{
...
SDL_STBIMG_DEF SDL_Texture*
STBIMG_LoadTexture_RW_noSeek(SDL_Renderer* renderer, SDL_IOStream* src, int freesrc)
{
