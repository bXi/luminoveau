...
// ─────────────────────────────────────────────────────────────────────────────
// RAII wrappers for raw SDL_GPU objects.
// SDL backend internal only — not part of the public engine API.
...
struct GpuTexture {
    ...
    GpuTexture() = default;
    GpuTexture(SDL_GPUDevice *dev, SDL_GPUTexture *tex)
        : handle(tex)
        , device(dev) { }
...
struct GpuBuffer {
    ...
    GpuBuffer() = default;
    GpuBuffer(SDL_GPUDevice *dev, SDL_GPUBuffer *buf)
        : handle(buf)
        , device(dev) { }
...
struct GpuTransferBuffer {
    ...
    GpuTransferBuffer(GpuTransferBuffer &&o) noexcept
        : handle(o.handle)
        , device(o.device) {
        o.handle = nullptr;
        o.device = nullptr;
...
struct GpuShader {
    ...
    GpuShader() = default;
    GpuShader(SDL_GPUDevice *dev, SDL_GPUShader *s)
        : handle(s)
        , device(dev) { }

    ~GpuShader() { reset(); }

    GpuShader(GpuShader &&o) noexcept
        : handle(o.handle)
        , device(o.device) {
        o.handle = nullptr;
        o.device = nullptr;
...
struct GpuGraphicsPipeline {
    ...
    GpuGraphicsPipeline(GpuGraphicsPipeline &&o) noexcept
        : handle(o.handle)
        , device(o.device) {
        o.handle = nullptr;
        o.device = nullptr;
...
struct GpuComputePipeline {
    ...
    GpuComputePipeline(GpuComputePipeline &&o) noexcept
        : handle(o.handle)
        , device(o.device) {
        o.handle = nullptr;
        o.device = nullptr;
...
struct GpuSampler {
    ...
    GpuSampler() = default;
    GpuSampler(SDL_GPUDevice *dev, SDL_GPUSampler *s)
        : handle(s)
        , device(dev) { }

    ~GpuSampler() { reset(); }

    GpuSampler(GpuSampler &&o) noexcept
        : handle(o.handle)
        , device(o.device) {
        o.handle = nullptr;
        o.device = nullptr;
