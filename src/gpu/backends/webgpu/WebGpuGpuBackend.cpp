...
bool WebGpuGpuBackend::init(void *windowHandle) {
    ...
    if (!m_instance) {
        LOG_WARNING("WebGpu init: wgpuCreateInstance returned null");
        return false;
    }
...
#endif
    if (!m_surface) {
        LOG_WARNING("WebGpu init: m_surface is null (canvas not found?)");
        return false;
    }
    ...
    if (!m_adapter) {
        ...
        m_adapter = requestAdapter(opts);
        if (m_adapter)
            LOG_WARNING("WebGpu init: adapter acquired without compatibleSurface filter");
    }
    if (!m_adapter) {
        LOG_WARNING("WebGpu init: requestAdapter returned null after fallbacks");
        return false;
    }
    ...
    if (!m_device) {
        LOG_WARNING("WebGpu init: requestDevice returned null (limits/features may exceed adapter caps)");
        return false;
    }
...
void WebGpuGpuBackend::shutdown() {
    ...
    if (m_currentSurfaceTex) {
        wgpuTextureRelease(m_currentSurfaceTex);
...
void WebGpuGpuBackend::submitCommandBuffer(GpuCmdBufferHandle cmd) {
    ...
    // seconds of running.
    for (auto &bg : cb->cleanup.tempBindGroups)
        wgpuBindGroupRelease(bg);
    for (auto b : cb->cleanup.tempBuffers)
        wgpuBufferRelease(b);
    for (auto &p : cb->cleanup.tempUniformBuffers)
        _recycleUniformBuffer(p.first, p.second);
...
GpuTextureHandle WebGpuGpuBackend::acquireSwapchainTexture(GpuCmdBufferHandle /*cmd*/,
...
#ifdef __EMSCRIPTEN__
    {
        ...
        if (w > 0 && h > 0 && (static_cast<uint32_t>(w) != m_swapchainWidth || static_cast<uint32_t>(h) != m_swapchainHeight)) {
            m_swapchainWidth  = static_cast<uint32_t>(w);
            m_swapchainHeight = static_cast<uint32_t>(h);
            EM_ASM({
                var c = document.querySelector('#canvas');
    ...
    if (surfTex.status == WGPUSurfaceGetCurrentTextureStatus_Outdated || surfTex.status == WGPUSurfaceGetCurrentTextureStatus_Lost) {
    ...
    if (surfTex.status != WGPUSurfaceGetCurrentTextureStatus_SuccessOptimal && surfTex.status != WGPUSurfaceGetCurrentTextureStatus_SuccessSuboptimal) {
...
GpuComputePassHandle WebGpuGpuBackend::beginComputePass(GpuCmdBufferHandle cmd,
    ...
    for (uint32_t i = 0; i < rwBufCount; ++i) {
        ...
        if (b)
            cp->pendingRWBuffers.push_back({ b->buffer, b->size });
    }
    ...
    for (uint32_t i = 0; i < rwTexCount; ++i) {
        ...
        if (t)
            cp->pendingRWTextures.push_back({ t->defaultView });
    }
...
void WebGpuGpuBackend::bindComputeSamplers(GpuComputePassHandle pass, uint32_t first,
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        te.binding     = base;
        te.textureView = tex ? tex->defaultView : nullptr;
        WGPUBindGroupEntry se {};
        se.binding = base + 1;
        se.sampler = smp ? smp->sampler : nullptr;
...
WGPUBindGroupLayout WebGpuGpuBackend::_makeStorageTexBGL(uint32_t count, WGPUShaderStageFlags vis,
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        entries[i].binding                      = i;
        entries[i].visibility                   = vis;
        entries[i].storageTexture.access        = (perBindingWriteOnly && perBindingWriteOnly[i]) ? WGPUStorageTextureAccess_WriteOnly : access;
...
GpuComputePipelineHandle WebGpuGpuBackend::createComputePipeline(const GpuComputePipelineCreateInfo &info) {
    ...
    pl->layout                  = wgpuDeviceCreatePipelineLayout(m_device, &plDesc);
    if (emptyBGLC)
        wgpuBindGroupLayoutRelease(emptyBGLC);
    ...
    if (errData.error) {
        ...
        if (pl->pipeline)
            wgpuComputePipelineRelease(pl->pipeline);
        if (pl->layout)
            wgpuPipelineLayoutRelease(pl->layout);
        for (auto &bgl : pl->bgLayouts) {
            if (bgl)
...
void WebGpuGpuBackend::releaseTexture(GpuTextureHandle handle) {
    ...
    if (t->defaultView)
        _evictSamplerBgsByView(t->defaultView);
    if (t->defaultView)
        wgpuTextureViewRelease(t->defaultView);
    for (auto &fv : t->faceViews)
        if (fv)
...
void WebGpuGpuBackend::releaseTransferBuffer(GpuTransferBufferHandle handle) {
    ...
    if (tb->downloadBuffer)
        wgpuBufferRelease(tb->downloadBuffer);
    delete tb;
}
...
void WebGpuGpuBackend::releaseSampler(GpuSamplerHandle handle) {
    ...
    if (s->sampler)
        _evictSamplerBgsBySampler(s->sampler);
    if (s->sampler)
...
void WebGpuGpuBackend::releaseShader(GpuShaderHandle handle) {
    ...
    if (sh->module)
        wgpuShaderModuleRelease(sh->module);
    delete sh;
}
...
void WebGpuGpuBackend::releaseGraphicsPipeline(GpuGraphicsPipelineHandle handle) {
    ...
    if (pl->layout)
        wgpuPipelineLayoutRelease(pl->layout);
    for (auto &bg : pl->emptyBG) {
        if (bg)
            wgpuBindGroupRelease(bg);
    }
    if (pl->emptyBGL)
        wgpuBindGroupLayoutRelease(pl->emptyBGL);
    for (auto &bgl : pl->bgLayouts) {
        if (bgl)
            wgpuBindGroupLayoutRelease(bgl);
    }
...
void WebGpuGpuBackend::uploadToTexture(GpuCmdBufferHandle /*cmd*/,
    ...
    default:
        bpp = 4;
        break; // RGBA8/BGRA8/R32F/RGB10A2/...
    }
...
WGPUTextureFormat WebGpuGpuBackend::toWGPU(GpuTextureFormat fmt) {
    ...
    case GpuTextureFormat::D24_Unorm_S8_Uint:
        return WGPUTextureFormat_Depth24PlusStencil8;
    case GpuTextureFormat::D32_Float_S8_Uint:
        return WGPUTextureFormat_Depth32FloatStencil8;
    case GpuTextureFormat::BC1_Unorm:
        return WGPUTextureFormat_BC1RGBAUnorm;
    case GpuTextureFormat::BC2_Unorm:
        return WGPUTextureFormat_BC2RGBAUnorm;
    case GpuTextureFormat::BC3_Unorm:
        return WGPUTextureFormat_BC3RGBAUnorm;
    case GpuTextureFormat::BC4_Unorm:
        return WGPUTextureFormat_BC4RUnorm;
    case GpuTextureFormat::BC5_Unorm:
...
WGPUTextureUsageFlags WebGpuGpuBackend::toWGPUUsage(GpuTextureUsage usage) {
    ...
    if (usage & GpuTextureUsage::ColorTarget)
        out |= WGPUTextureUsage_RenderAttachment;
    if (usage & GpuTextureUsage::DepthStencilTarget)
        out |= WGPUTextureUsage_RenderAttachment;
    if (usage & GpuTextureUsage::StorageRead)
        out |= WGPUTextureUsage_StorageBinding;
    if (usage & GpuTextureUsage::StorageWrite)
        out |= WGPUTextureUsage_StorageBinding;
    if (usage & GpuTextureUsage::Transfer)
        out |= WGPUTextureUsage_CopySrc | WGPUTextureUsage_CopyDst;
    return out;
}
...
WGPUBufferUsageFlags WebGpuGpuBackend::toWGPUUsage(GpuBufferUsage usage) {
    ...
    if (usage & GpuBufferUsage::StorageRead)
        out |= WGPUBufferUsage_Storage;
    if (usage & GpuBufferUsage::StorageWrite)
        out |= WGPUBufferUsage_Storage;
...
WGPUAddressMode WebGpuGpuBackend::toWGPU(GpuSamplerAddressMode m) {
    ...
    case GpuSamplerAddressMode::MirroredRepeat:
...
WGPUVertexFormat WebGpuGpuBackend::toWGPU(GpuVertexElementFormat fmt) {
    switch (fmt) {
    case GpuVertexElementFormat::Float:
    ...
    case GpuVertexElementFormat::Byte2Norm:
        return WGPUVertexFormat_Snorm8x2;
    case GpuVertexElementFormat::Byte4Norm:
        return WGPUVertexFormat_Snorm8x4;
    case GpuVertexElementFormat::UByte2Norm:
        return WGPUVertexFormat_Unorm8x2;
    case GpuVertexElementFormat::UByte4Norm:
        return WGPUVertexFormat_Unorm8x4;
    case GpuVertexElementFormat::Short2:
        return WGPUVertexFormat_Sint16x2;
    case GpuVertexElementFormat::Short4:
        return WGPUVertexFormat_Sint16x4;
    case GpuVertexElementFormat::Short2Norm:
        return WGPUVertexFormat_Snorm16x2;
    case GpuVertexElementFormat::Short4Norm:
        return WGPUVertexFormat_Snorm16x4;
    case GpuVertexElementFormat::Half2:
        return WGPUVertexFormat_Float16x2;
    case GpuVertexElementFormat::Half4:
        return WGPUVertexFormat_Float16x4;
    case GpuVertexElementFormat::UInt:
        return WGPUVertexFormat_Uint32;
    case GpuVertexElementFormat::UInt2:
        return WGPUVertexFormat_Uint32x2;
    case GpuVertexElementFormat::UInt4:
        return WGPUVertexFormat_Uint32x4;
    case GpuVertexElementFormat::Int:
        return WGPUVertexFormat_Sint32;
    case GpuVertexElementFormat::Int2:
        return WGPUVertexFormat_Sint32x2;
    case GpuVertexElementFormat::Int4:
...
GpuTextureFormat WebGpuGpuBackend::fromWGPU(WGPUTextureFormat fmt) {
    ...
    case WGPUTextureFormat_RGBA8UnormSrgb:
        return GpuTextureFormat::R8G8B8A8_Unorm_SRGB;
    case WGPUTextureFormat_BGRA8UnormSrgb:
        return GpuTextureFormat::B8G8R8A8_Unorm_SRGB;
    case WGPUTextureFormat_Depth32Float:
        return GpuTextureFormat::D32_Float;
    default:
        return GpuTextureFormat::R8G8B8A8_Unorm;
    }
}
