...
// the public header never pulls in SDL_net / socket types.
...
private:
    ...
    static constexpr uint32_t _fnv1a(const char *s, uint32_t h = 2166136261u) {
        return (*s == 0) ? h : _fnv1a(s + 1, (h ^ static_cast<uint8_t>(*s)) * 16777619u);
    }
    // Stable per-type id from the compiler's decorated function name.
    template <typename T>
    static constexpr uint32_t _typeId() {
#if defined(_MSC_VER)
    ...
    NetTransport::ITransport                                                      *_transport = nullptr;
