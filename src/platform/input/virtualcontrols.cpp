...
void VirtualControls::HandleTouchEvent(const SDL_Event *event) {
    ...
    case SDL_EVENT_FINGER_DOWN: {
        ...
        if (m_joystickMode != JoystickMode::DISABLED && !m_joystick.isActive && IsTouchInJoystickArea(touchPos)) {
        ...
        // Check button activation, then the look region.
            ...
            else if (m_joystickRightMode != JoystickMode::DISABLED && !m_joystickRight.isActive && IsTouchInRightJoystickArea(touchPos)) {
...
void VirtualControls::UpdateMouse() {
    ...
    if (mouseDown) {
        ...
        if (m_lookEnabled && !m_joystick.isActive && IsInLookRegion(mousePos)
            ...
            if (!m_lookActive) {
                m_lookActive   = true;
                m_lookFinger   = MOUSE_FINGER_ID;
                m_lookLast     = mousePos;
                m_lookStart    = mousePos;
                m_lookMaxDist2 = 0.0f;
...
void VirtualControls::RenderJoystick() {
    if (m_joystickMode != JoystickMode::DISABLED && !(m_joystickMode == JoystickMode::RELATIVE && !m_joystick.isActive)) {
    ...
    if (m_joystickRightMode != JoystickMode::DISABLED && !(m_joystickRightMode == JoystickMode::RELATIVE && !m_joystickRight.isActive)) {
