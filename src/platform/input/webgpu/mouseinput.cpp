...
static void lumiEnsurePointerHooks() {
    EM_ASM({
        ...
        if (c0) c0.style.touchAction = 'none';       // don't let the browser eat drags for scroll/zoom
        ...
        function recompute() {
            var touches = []; var hasMouse = false; var mouseBtns = 0;
            pressed.forEach(function (p) {
                if (p.type === 'mouse') { hasMouse = true; mouseBtns = p.buttons; }
                else touches.push(p);
            });
            var mask = 0;
            // Pointer e.buttons: 1=left,2=right,4=middle → SDL mask: 1=left,2=middle,4=right.
            if (hasMouse) { if (mouseBtns & 1) mask |= 1; if (mouseBtns & 2) mask |= 4; if (mouseBtns & 4) mask |= 2; }
            if (touches.length === 1) mask |= 1;     // single finger acts as the left button
            if (touches.length >= 2) window._lumiPos = ({ x: (touches[0].x + touches[1].x) / 2,
                                                          y: (touches[0].y + touches[1].y) / 2 });
            window._lumiBtns = mask;
        }
        window.addEventListener('pointerdown', function (e) {
            var xy = canvasXY(e); if (!xy) return;
            pressed.set(e.pointerId, { x: xy.x, y: xy.y, type: e.pointerType, buttons: e.buttons });
            window._lumiPos = xy; recompute();
        }, true);
        window.addEventListener('pointermove', function (e) {
            var xy = canvasXY(e); if (!xy) return;
            if (pressed.has(e.pointerId)) { var p = pressed.get(e.pointerId); p.x = xy.x; p.y = xy.y; p.buttons = e.buttons; }
            var touchCount = 0; pressed.forEach(function (p) { if (p.type !== 'mouse') touchCount++; });
            if (touchCount < 2) window._lumiPos = xy;   // pinch midpoint is set in recompute()
            recompute();
        }, true);
        function up(e) { pressed.delete(e.pointerId); recompute(); }
