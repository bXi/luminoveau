...
void ShaderRenderPass::release(bool logRelease) {
    ...
    if (fs.texture.gpuTexture) {
        SDL_ReleaseGPUTexture(Renderer::GetDevice(), reinterpret_cast<SDL_GPUTexture*>(fs.texture.gpuTexture));
...
bool ShaderRenderPass::init(
    ...
    vertex_shader   = reinterpret_cast<SDL_GPUShader*>(vertShader.gpuShader);
    fragment_shader = reinterpret_cast<SDL_GPUShader*>(fragShader.gpuShader);
    ...
    resultTexture = reinterpret_cast<SDL_GPUTexture*>(AssetHandler::CreateEmptyTexture(Window::GetPhysicalSize()).gpuTexture);
    inputTexture = reinterpret_cast<SDL_GPUTexture*>(AssetHandler::CreateEmptyTexture(Window::GetPhysicalSize()).gpuTexture);
    ...
    std::vector<SDL_GPUColorTargetDescription> color_target_descriptions(fragShader.samplerCount, SDL_GPUColorTargetDescription{
        ...
        .blend_state = toSDL(GpuPresets::AlphaBlend),
    });
...
void ShaderRenderPass::render(
    ...
    // STEP 1: Copy window region from desktop-sized framebuffer to window-sized inputTexture
    {
        ...
        SDL_GPUTextureSamplerBinding binding{
            .texture = reinterpret_cast<SDL_GPUTexture*>(framebuffer->fbContent),
            .sampler = reinterpret_cast<SDL_GPUSampler*>(Renderer::GetSampler(ScaleMode::Linear)),
        };
    ...
    {
    SDL_GPURenderPass* sdl_rp = SDL_BeginGPURenderPass(cmd_buffer, color_target_info.data(), color_target_info.size(), nullptr);
    ...
    assert(sdl_rp);
    {
        ...
        std::vector<SDL_GPUTextureSamplerBinding> texture_sampler_bindings(fragShader.samplerCount, SDL_GPUTextureSamplerBinding{
            .texture = inputTexture,
            .sampler = reinterpret_cast<SDL_GPUSampler*>(Renderer::GetSampler(AssetHandler::GetDefaultTextureScaleMode())),
        });
        ...
        for (const auto &sampler: foundSamplers) {
            if (fragShader.frameBufferToSamplerMapping.contains(sampler)) {
                ...
                texture_sampler_bindings[i].texture = reinterpret_cast<SDL_GPUTexture*>(fb->fbContent);
            }
            i++;
        }

        SDL_BindGPUFragmentSamplers(sdl_rp, 0, texture_sampler_bindings.data(), fragShader.samplerCount);
        SDL_DrawGPUPrimitives(sdl_rp, 6, 1, 0, 0);
    }

    SDL_EndGPURenderPass(sdl_rp);
    }

    _renderShaderOutputToFramebuffer(cmd_buffer, reinterpret_cast<SDL_GPUTexture*>(framebuffer->fbContent), resultTexture, camera);
