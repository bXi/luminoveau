...
void Renderer::_initRendering() {
    ...
    // Additive pipeline for renderToScreen FBs that want additive compositing (ONE + ONE)
    ...
    m_rendertotexturepipeline_additive = SDL_CreateGPUGraphicsPipeline(Renderer::GetDevice(), &rtt_pipeline_create_info);
    ...
    #ifdef LUMINOVEAU_WITH_IMGUI
    ImGuiIntegration::InitRenderer(m_device, Window::GetWindow());
    #endif
}
...
void Renderer::_close() {
    ...
    if (m_rendertotexturepipeline_additive) {
        SDL_ReleaseGPUGraphicsPipeline(m_device, m_rendertotexturepipeline_additive);
        m_rendertotexturepipeline_additive = nullptr;
    }
    ...
    for (auto &[fbName, framebuffer]: frameBuffers) {
        ...
        if (useMSAA && !framebuffer->noMSAA) {
            GpuTextureCreateInfo msaaColorInfo{
                ...
                .format        = fromSDL(swapchainFormat),
...
void Renderer::renderFrameBuffer(SDL_GPUCommandBuffer *cmd_buffer) {
    ...
    auto *framebuffer = reinterpret_cast<SDL_GPUTexture*>(framebufferObj->fbContent);
    ...
    for (const auto& [fbName, framebuffer] : frameBuffers) {
        if (framebuffer->renderToScreen) {
            auto* pipeline = framebuffer->additiveBlend
                ? m_rendertotexturepipeline_additive
                : m_rendertotexturepipeline;
...
void Renderer::_createFrameBuffer(const std::string &fbname, uint32_t width, uint32_t height) {
    ...
    if (it == frameBuffers.end()) {
        // Use provided size if non-zero, otherwise fall back to desktop size
