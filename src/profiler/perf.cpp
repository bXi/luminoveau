...
std::string queryCpuName() {
    ...
    if (sysctlbyname("machdep.cpu.brand_string", buf, &sz, nullptr, 0) == 0 && buf[0])
        return buf;
    return "Unknown CPU";
#elif defined(__linux__)
...
std::string queryGpuName() {
    ...
    if (vkCreateInstance && vkEnumerate && vkGetInstanceProc) {
        struct AppInfo {
            ...
            uint32_t    ev;
            uint32_t    api;
        } app { 0, nullptr, nullptr, 0, nullptr, 0, (1u << 22) };
        struct CI {
            int                sType;
            const void        *pNext;
            uint32_t           flags;
        ...
        if (vkCreateInstance(&ci, nullptr, &inst) == 0 && inst) {
            ...
            uint32_t count      = 0;
            vkEnumerate(inst, &count, nullptr);
            if (count && vkGetProps) {
                if (count > 8)
                    count = 8;
                void *devs[8] = { nullptr };
                vkEnumerate(inst, &count, devs);
                for (uint32_t i = 0; i < count && devs[i]; i++) {
                    unsigned char props[1024] = { 0 };
                    vkGetProps(devs[i], props);
...
const std::string &cpuName() {
    static std::string s = queryCpuName();
    return s;
}
const std::string &gpuName() {
    static std::string s = queryGpuName();
    return s;
}
...
static float steppedCeil(float v) {
    ...
    float n    = v / base; // 1 .. 10
    float step = (n < 1.5f) ? 0.25f : (n < 3.0f) ? 0.5f
        : (n < 6.0f)                             ? 1.0f
                                                 : 2.0f;
    step *= base;
...
void Perf::_render() {
    ...
    drawGraph(gy, _frameMs, frameC, std::max(steppedCeil(peakFps * 1.05f), 30.0f), true, "fps");
    gy += graphH + pad;
    drawGraph(gy, _cpuHist, cpuC, std::max(steppedCeil((float)maxCpu * 1.1f), 1.0f), false, "cpu");
    gy += graphH + pad;
    drawGraph(gy, _gpuHist, gpuC, std::max(steppedCeil((float)maxGpu * 1.1f), 1.0f), false, "gpu");
    gy += graphH + pad;
