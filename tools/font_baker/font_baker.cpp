...
int main(int argc, char **argv) {
    ...
    if (!ft) {
        std::fprintf(stderr, "font_baker: FreeType init failed\n");
        return 1;
    }
    ...
    if (!font) {
        std::fprintf(stderr, "font_baker: font load failed\n");
        return 1;
    }
    ...
    for (auto &g : glyphs)
        g.edgeColoring(&msdfgen::edgeColoringInkTrap, maxCornerAngle, 0);
    ...
    if (!f) {
        std::fprintf(stderr, "font_baker: cannot open %s\n", outPath);
        return 1;
    }
    std::fprintf(f,
